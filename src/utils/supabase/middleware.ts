...
import { SupabaseFactory } from './singleton.server';

export async function updateSession(request: NextRequest) {
  
  SupabaseFactory.destroyInstance();
  const [supabase, response] = SupabaseFactory.getInstance(request);
...
/*export async function updateSession(request: NextRequest) {
  ...
  const supabase = createServerClient(
    ...
    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!,
    {
      cookies: {
        getAll() {
          console.log('middleware.ts', 'getting all cookies...')
          return request.cookies.getAll()
        },
        setAll(cookiesToSet) {
          console.log('middleware.ts', 'setting all cookies...', { cookiesToSet })
